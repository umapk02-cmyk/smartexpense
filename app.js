...
const CATEGORIES = [
  ...
  "Wallet / FASTag Recharge","Miscellaneous"
];
const SOURCES = [
...
const ICONS = {
  ...
  "Gardening":"🌿","Gifts":"🎁","Wallet / FASTag Recharge":"💳","Miscellaneous":"📦"
};
...
function MainApp({user,onLogout}) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const header = ["Date","Amount","Description","Category","Account","Entered By"];
      const drows  = data.map(r=>[r.date?.slice(0,10)||"",r.amount,r.description,r.category,r.source,r.enteredBy]);
      const csv    = [header,...drows].map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\r\n");
      const a      = document.createElement("a");
      a.href       = "data:text/csv;charset=utf-8,\uFEFF"+encodeURIComponent(csv);
      a.download   = `expenses_${exportFrom}_to_${exportTo}.csv`;
      a.click();
  ...
  return e("div",{style:{minHeight:"100vh",background:"#f0f4f8",fontFamily:"system-ui,sans-serif",fontSize:13}},
    ...
    e("div",{style:{background:"linear-gradient(135deg,#1F4E79,#2E75C2)",boxShadow:"0 2px 12px rgba(0,0,0,0.2)"}},
      ...
      e("div",{style:{display:"flex",gap:2,padding:"0 14px",overflowX:"auto"}},
        ["entry","dashboard","summary","export"].map(t=>e("button",{key:t,onClick:()=>setTab(t),style:{padding:"7px 13px",borderRadius:"7px 7px 0 0",border:"none",cursor:"pointer",fontSize:11,fontWeight:600,whiteSpace:"nowrap",background:tab===t?"#f0f4f8":"transparent",color:tab===t?"#1F4E79":"rgba(255,255,255,0.7)"}},{"entry":"📝 Entry","dashboard":"🎯 Dashboard","summary":"📊 Summary","export":"📥 Export"}[t]))
      )
    ),
    ...
    e("div",{style:{maxWidth:700,margin:"0 auto",padding:"12px 8px"}},
      ...
      tab==="entry"&&e("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        ...
        e("div",{style:{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}},
          ...
          e("div",{style:{marginBottom:12}},
            ...
            e("input",{id:"desc-input",type:"text",value:newDesc,
              ...
              style:{width:"100%",padding:"10px 14px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",background:"#faf5ff",boxSizing:"border-box"}})
          ),
        ...
        e("div",{style:{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}},
          e("div",{style:{fontSize:12,fontWeight:700,color:"#1F4E79",marginBottom:dayRows.length>0?12:0}},
          ...
          dayRows.length===0
            ...
            :dayRows.map(row=>{
              ...
              return e("div",{key:row.id,onClick:()=>setEditRow(row),style:{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid #f3f4f6",cursor:"pointer"}},
                ...
                e("div",{style:{fontSize:14,fontWeight:800,color:src?.color||"#111",minWidth:70,textAlign:"right"}},"₹"+fmtN(row.amount)),
        ...
        // Monthly budgets
        e("div",{style:{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}},
          e("div",{style:{fontSize:12,fontWeight:700,color:"#1F4E79",marginBottom:12}},"🎯 Monthly Budgets"),
          e("div",{style:{display:"flex",flexDirection:"column",gap:8}},
            CATEGORIES.map(cat=>{
              const actual=fullT[cat]?.total||0,budget=parseFloat(budgets[cat])||0,over=budget>0&&actual>budget;
              return e("div",{key:cat,style:{display:"flex",alignItems:"center",gap:10}},
                e("div",{style:{flex:1,fontSize:11,color:"#374151"}},ICONS[cat]+" "+cat),
                actual>0&&e("div",{style:{fontSize:11,color:over?"#dc2626":"#6b7280",minWidth:60,textAlign:"right"}},fmt(actual)),
                e("input",{type:"number",min:"0",value:budgets[cat]||"",placeholder:"Budget",
                  onChange:ev=>setBudgets(p=>({...p,[cat]:ev.target.value})),
                  onBlur:handleBudgetBlur,
                  style:{width:90,padding:"5px 8px",border:"1px solid #fde68a",borderRadius:7,fontSize:11,textAlign:"right",background:"#fffbeb",color:"#92400e",fontWeight:600,outline:"none"}})
              );
            })
          )
        )
      ...
      tab==="dashboard"&&e("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        e("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:10}},
          [{label:"Total Spent",val:fmt(fullGrand),sub:activeDay+" of "+days+" days",color:"#2E75C2",icon:"💰"},
           {label:"Budget",val:totalBud>0?fmt(totalBud):"Not set",sub:"Monthly limit",color:"#16a34a",icon:"🎯"},
           {label:rem!==null&&rem<0?"Over Budget":"Remaining",val:rem!==null?fmt(Math.abs(rem)):"—",sub:rem===null?"Set budgets in Entry":rem>=0?"Left to spend":"⚠️ Exceeded",color:rem===null?"#9ca3af":rem>=0?"#16a34a":"#dc2626",icon:rem!==null&&rem<0?"🚨":"✅"},
           {label:"Daily Average",val:fmt(fullGrand/Math.max(1,activeDay)),sub:"This month",color:"#7c3aed",icon:"📅"},
          ].map((k,i)=>e("div",{key:i,style:{background:"#fff",borderRadius:12,padding:"12px 14px",boxShadow:"0 1px 6px rgba(0,0,0,0.07)",borderLeft:"4px solid "+k.color}},
            e("div",{style:{display:"flex",justifyContent:"space-between"}},
              e("div",null,e("div",{style:{fontSize:9,color:"#6b7280",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}},k.label),e("div",{style:{fontSize:18,fontWeight:800,color:k.color,margin:"2px 0"}},k.val),e("div",{style:{fontSize:9,color:"#9ca3af"}},k.sub)),
              e("span",{style:{fontSize:20}},k.icon)
            )
          ))
        ),
        ...
        e("div",{style:{background:"linear-gradient(135deg,#1F4E79,#7c3aed)",borderRadius:14,padding:16}},
          e("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}},
            e("div",null,e("div",{style:{fontSize:13,fontWeight:800,color:"#fff"}},"🤖 AI Deep Analysis"),e("div",{style:{fontSize:10,color:"rgba(255,255,255,0.6)"}},"Advisor-level insights")),
            e("button",{onClick:fetchAI,disabled:aiLoading||fullGrand===0,style:{background:"#fff",color:"#1F4E79",border:"none",borderRadius:9,padding:"8px 16px",fontSize:12,fontWeight:700,cursor:fullGrand>0?"pointer":"not-allowed",opacity:fullGrand===0?0.5:1}},aiLoading?"Analyzing…":"✨ Get AI Insights")
          ),
          aiInsights.length>0&&e("div",{style:{display:"flex",flexWrap:"wrap",gap:10,marginTop:12}},aiInsights.map((ins,i)=>e("div",{key:i,style:{background:"#fff",borderRadius:12,padding:"12px 14px",border:"1.5px solid "+ins.color+"33",flex:"1 1 200px"}},e("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}},e("span",{style:{fontSize:11,fontWeight:700,color:"#374151"}},ins.icon+" "+ins.title),ins.tag&&e("span",{style:{fontSize:9,background:ins.color,color:"#fff",padding:"2px 7px",borderRadius:20,fontWeight:700}},ins.tag)),e("div",{style:{fontSize:12,color:"#1f2937",lineHeight:1.55}},ins.body)))),
          fullGrand===0&&e("div",{style:{fontSize:11,color:"rgba(255,255,255,0.5)",marginTop:10,fontStyle:"italic"}},"Enter expenses first.")
        ),
      ...
      tab==="export"&&e("div",{style:{display:"flex",flexDirection:"column",gap:12}},
        e("div",{style:{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}},
          ...
          e("button",{onClick:handleExport,disabled:exporting,style:{width:"100%",padding:12,background:"#1F4E79",color:"#fff",border:"none",borderRadius:9,fontSize:13,fontWeight:700,cursor:"pointer"}},exporting?"Fetching…":"📥 Download CSV")
        ),
